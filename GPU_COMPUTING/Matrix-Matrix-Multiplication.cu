  ...
  //@@ Insert code to implement matrix multiplication here
	...
	for (int k = 0; k < (TILE_DIM + numAColumns - 1)/TILE_DIM; k++) {
        ...
        for (int n = 0; n < TILE_DIM; ++n) 
            if ((k*TILE_DIM + n < numAColumns && Row < numARows) && (k*TILE_DIM + n < numBRows && Col < numBColumns))
                CValue += A[Row*numAColumns + k*TILE_DIM + n] * B[(k*TILE_DIM + n)*numBColumns + Col];
...
int main(int argc, char **argv) {
  ...
  //@@ Initialize the grid and block dimensions here
	dim3 DimGrid((numCColumns + 16 - 1) / 16, (numCRows + 16 - 1) / 16);
