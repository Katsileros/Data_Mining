...
__global__ void loop(float *input, float *aux, int len) {
...
int main(int argc, char ** argv) {
    ...
    int numBlocks = ceil((float)numElements/(BLOCK_SIZE/2));
    ...
    cudaDeviceSynchronize();
    loop<<<dimGrid, dimBlock>>>(deviceOutput, deviceAuxScannedArray, numElements);
